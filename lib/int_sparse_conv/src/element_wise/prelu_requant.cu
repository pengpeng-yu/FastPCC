...
template <typename T>
__inline__ __device__ T scalar_prelu_requant(
  const int32_t input,
...
template <typename T>
__global__ void prelu_requant_kernel(
  ...
  uint64_t N, uint64_t Ch)
{
  ...
  for (; idx < total; idx += stride) {
    uint64_t ch = idx % Ch;
    out[idx] = scalar_prelu_requant<T>(input[idx], slope_val, requant_mul[ch], zero_point[0], right_shift);
  }
}

template <typename T>
at::Tensor prelu_requant(
  ...
  at::ScalarType out_dtype)
{
  ...
  TORCH_CHECK(slope.device() == device && requant_mul.device() == device && zero_point.device() == device);
  TORCH_CHECK(input.is_contiguous() && slope.is_contiguous() && requant_mul.is_contiguous());
  TORCH_CHECK(input.dim() == 2 && requant_mul.dim() == 1);

  int64_t N = input.size(0);
  int64_t Ch = input.size(1);
  TORCH_CHECK(N > 0 && Ch > 0 && right_shift >= 0);
  TORCH_CHECK(1 == slope.numel() && Ch == requant_mul.size(0) && 1 == zero_point.numel());
  ...
  prelu_requant_kernel<T><<<blocks, threads, 0, stream>>>(
...
at::Tensor prelu_requant_to_int8(
  ...
  const int right_shift)
{
  return prelu_requant<int8_t>(input, slope, requant_mul, zero_point, right_shift, at::kChar);
}
...
at::Tensor prelu_requant_to_int16(
  ...
  const int right_shift)
{
  return prelu_requant<int16_t>(input, slope, requant_mul, zero_point, right_shift, at::kShort);
}
...
at::Tensor prelu_requant_to_int32(
  ...
  const int right_shift)
{
  return prelu_requant<int32_t>(input, slope, requant_mul, zero_point, right_shift, at::kInt);
}
