...
def read_as_3d_array(fp, fix_coords=True):
    ...
    if fix_coords:
        # xzy to xyz TODOthe right thing
...
def read_as_coord_array(fp, fix_coords=True, dtype=np.int64):
    ...
    sz = np.prod(dims)
    index, end_index = 0, 0
    end_indices = np.cumsum(counts, dtype=dtype)
    ...
    nz_voxels = np.array(nz_voxels, dtype=dtype)
    # TODOare these dims correct?
    # according to docs,
...
# TODOensure this is right when dims are not all same
# """
