...
class GeoLosslessEntropyModel(nn.Module):
    def __init__(self,
                 ...
                 encoder: nn.Module,
                 residual_block: Union[nn.Module, nn.ModuleList],
                 decoder_block: Union[nn.Module, nn.ModuleList],
    ...
    def get_sub_residual_block(self, idx):
        if hasattr(self.residual_block, '__getitem__'):
            return self.residual_block[idx]
        else:
            return self.residual_block

    def get_sub_decoder_block(self, idx):
        if hasattr(self.decoder_block, '__getitem__'):
            return self.decoder_block[idx]
        else:
    ...
    def forward(self, y_top: ME.SparseTensor, *args, **kwargs):
        ...
        *strided_fea_list, bottom_fea = strided_fea_list
        ...
        for idx in range(strided_fea_list_len - 1, -1, -1):
            ...
            sub_residual_block = self.get_sub_residual_block(idx)
            sub_decoder_block = self.get_sub_decoder_block(idx)
            ...
            if idx <= self.skip_encoding_fea:
                ...
                fea_pred_res_tilde, fea_loss_dict = self.indexed_entropy_model_fea(
                    sub_residual_block(torch.cat((fea.F[None], fea_pred[None]), dim=2)),
                    fea_indexes, is_first_forward=idx == strided_fea_list_len - 1,
                    x_grad_scaler_for_bits_loss=self.upper_fea_grad_scaler_for_bits_loss
                )
                lower_fea_tilde = ME.SparseTensor(
                    features=sub_decoder_block(torch.cat((fea_pred_res_tilde, fea_pred[None]), dim=2))[0],
                    coordinate_map_key=coord_target_key,
                    coordinate_manager=cm
                )
        ...
        for idx in range(strided_fea_list_len - 1, -1, -1):
            ...
            if idx <= self.skip_encoding_fea:
                ...
                (fea_bytes,), fea_pred_res_recon = self.indexed_entropy_model_fea.compress(
                    sub_residual_block(torch.cat((fea.F[None], fea_pred[None]), dim=2)), fea_indexes,
                )
                lower_fea_recon = ME.SparseTensor(
                    features=sub_decoder_block(torch.cat((fea_pred_res_recon, fea_pred[None]), dim=2))[0],
                    coordinate_map_key=coord_recon_key,
                    coordinate_manager=cm
                )
    ...
    def decompress(self, concat_bytes: bytes,
        ...
        for idx in range(fea_bytes_list_len - 1, -1, -1):
            ...
            if idx <= self.skip_encoding_fea:
                ...
                fea_info_pred = sub_hyper_decoder_fea(lower_fea_recon, coord_recon_key).F
                fea_recon_channels = lower_fea_recon.shape[1]
                fea_pred, pre_fea_indexes = torch.split(
                    fea_info_pred,
                ...
                lower_fea_recon = ME.SparseTensor(
                    features=sub_decoder_block(torch.cat((lower_fea_pred_res_recon, fea_pred[None]), dim=2))[0],
                    coordinate_map_key=coord_recon_key,
                    coordinate_manager=cm
                )
    ...
    def get_coord_mask(
            ...
            indexes: ME.SparseTensor) -> ME.SparseTensor:
        cm = y_coords_tuple[1]
        assert cm is indexes.coordinate_manager
        target_key = y_coords_tuple[0]
        kernel_map = cm.kernel_map(indexes.coordinate_map_key, target_key, kernel_size=1)
...
class GeoLosslessNoisyDeepFactorizedEntropyModel(GeoLosslessEntropyModel):
    def __init__(self,
        ...
        super(GeoLosslessNoisyDeepFactorizedEntropyModel, self).__init__(
            bottom_fea_entropy_model, encoder, residual_block, decoder_block,
            hyper_decoder_coord, hyper_decoder_fea,
