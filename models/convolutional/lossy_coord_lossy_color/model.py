...
class PCC(nn.Module):
    ...
    def get_sparse_pc(self, xyz: torch.Tensor, color: torch.Tensor) -> ME.SparseTensor:
        ...
        if not self.training:
            ...
            morton_indices = torch.argsort(morton_encode_magicbits(xyz[:, 1:]))
            xyz = xyz[morton_indices]
            color = color[morton_indices]
    ...
    def train_forward(self, batched_coord: torch.Tensor, batched_color: torch.Tensor,
        ...
        for key in loss_dict:
            ...
            if key.startswith('coord_') and key.endswith('_recon_loss'):
    ...
    def test_forward(self, pc_data: PCData):
        ...
        ret = self.evaluator.log(
            ...
            pred_color=color_recon,
            target_color=pc_data.color if not_part else pc_data.color[0],
            extra_info_dict={
    ...
    def compress(self, batched_coord: torch.Tensor,
                 batched_color: torch.Tensor) -> bytes:
        coord_offset = batched_coord[:, 1:].amin(0)
        sparse_pc = self.get_sparse_pc(batched_coord - F.pad(coord_offset, (1, 0)), batched_color)
        feature, points_num_list = self.encoder(sparse_pc)
    ...
    def compress_partitions(self, batched_coord: List[torch.Tensor],
                            batched_color: List[torch.Tensor]) -> bytes:
        compressed_bytes_list = []
        for idx in range(1, len(batched_coord)):
            compressed_bytes = self.compress(batched_coord[idx], batched_color[idx])
    ...
    def decompress_partitions(self, concat_bytes: bytes) -> Tuple[torch.Tensor, torch.Tensor]:
        coord_recon_list = []
        color_recon_list = []
