...
class TransformerBlock(nn.Module):
    def __init__(self, d_in, d_model, nneighbor, d_out=None) -> None:
        ...
        self.d_in, self.d_model, self.k, self.d_out = d_in, d_model, nneighbor, d_out
        
    # xyz: b, n, 3, features: b, n, d_in
    def forward(self, xyz, features, require_attn=False):
        knn_idx = torch.cdist(xyz, xyz).topk(self.k, dim=-1, largest=False, sorted=False)[1]  # knn_idx b, n, k  # TODO: precompute knn_idx to avoid duplicated computation
